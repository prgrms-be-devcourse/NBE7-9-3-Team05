...
interface StorageApi {
    ...
    @ApiResponses(
        value = [
            ApiResponse(
                responseCode = StorageHttp.CREATE_AWS_URL_SUCCESS_CODE,
                description = StorageHttp.CREATE_AWS_URL_SUCCESS_MESSAGE,
                content = [
                    Content(schema = Schema(implementation = UploadUrlResponse::class))
                ]
            )
        ]
    )
    fun createUploadUrl(@RequestBody @Valid request: CreateUploadUrlRequest): ResponseData<UploadUrlResponse>
    ...
    @ApiResponses(
        value = [
            ApiResponse(
                responseCode = StorageHttp.CREATE_CDN_URL_SUCCESS_CODE,
                description = StorageHttp.CREATE_CDN_URL_SUCCESS_MESSAGE,
                content = [
                    Content(schema = Schema(implementation = CdnUrlResponse::class))
                ]
            )
        ]
    )
    fun signCdnUrl(@RequestParam("key") key: String): ResponseData<CdnUrlResponse>
}
