...
interface ChallengeParticipantApi {
    ...
    @Operation(
        ...
        responses = [
            ApiResponse(
                ...
                responseCode = "400",
                description = "이미 참가한 유저 / 정원 초과"
            ), ApiResponse(
                responseCode = "404",
                description = "존재하지 않는 방 또는 유저")
        ]
    )

    fun joinChallengeRoom(
    ...
    fun leaveChallengeRoom(
        @PathVariable("roomId") roomId: @NotNull Long
    ): ResponseData<Void>

    @GetMapping("/{roomId}/status")
    @Operation(
        summary = "사용자의 운동방 가입여부 조회",
